...
#      print "$k ($no)\n"; $k++;
  ...
  # cluster member of the last detected cluster
    ...
    if ($clstr_type = ".clstr" ) {
      # append the seq-id WITHOUT the _pepid termination to an array 
      # of current cluster members so that we can identify the fasta 
      # later (the original fasta lacks the _pepid part)
      if ($ll =~ /(aa|nt), >(.+)_[0-9]+\.\.\./) { 
          #print $2 ; 
          push(@ids, $2); $no++; 
      }
    }
    if ($clstr_type = ".uclstr") {
      # with current uclust we do not get the _pepid part that identifies
      # the peptide fragment number within the protein sequence
      # THIS SOLVES THE PROBLEM WITH SEQID CONTAINING AN '_' AND NO PEPID
      if ($ll =~ /(aa|nt), >(.+)\.\.\./) { 
          #print $2 ;
          push(@ids, $2); $no++; 
      } else { die "Wrong format $ll"; }
...
while($ll=<FASTA>) {
  # if it is a new entry, get the id (any non space after the > until a ' ')
  if ($ll=~ /^>(\S+)/) {
    # get the ID
    my $id = $1;
