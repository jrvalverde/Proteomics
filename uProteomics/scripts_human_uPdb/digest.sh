#   !/bin/bash
#
#   DIGEST a folder with FASTA databases/files and select
# only peptides withing a size range (for all, use 
# 0..some-bit-number)
#   This is based on digest.03.R with minor adjustments
# for generality

INPUT=${1:-ucat.1}
OUTPUT=${2:-${INPUT}_digested}
MINSIZE=${3:-8}
MAXSIZE=${4:-30}

#echo
#echo "$1 - $2 - $3 - $4"
#echo $INPUT $OUTPUT $MINSIZE $MAXSIZE
#exit

R --nosave --vanilla <<END
...
minl <- $MINSIZE       # minimum peptide length (val. included)
maxl <- $MAXSIZE      # maximum peptide length (val. included)
