...
## load (and install if needed) CRAN packages
cran_packages <- c(
	"RColorBrewer", 
	"ggplot2", 
	"reshape2", 
...
# load BioConductor packages (and install if needed)
library.bioc("MSnbase")
library.bioc("MSnSet.utils")
library.bioc("RColorBrewer") ## Color palettes
library.bioc("ggplot2")  ## Convenient and nice plotting
library.bioc("reshape2") ## Flexibly reshape data
library.bioc("vsn")
library.bioc("MALDIquant")
library.bioc("MALDIquantForeign")
library.bioc("BRAIN")
library.bioc("Rdisop")
library.bioc("OrgMassSpecR")
library.bioc("isobar")
library.bioc("DEP")
library.bioc("msmsTests")
library.bioc("synapter")
library.bioc("qvalue")


# load (and install if needed) GitHub packages
...
# Input file name(s) with the relevant proteomic results data
...
# for single file
#	prefix that will allow us to identify the columns containing
...
###outFilePrefix <- "SclAbun"
outFilePrefix <- "NormAbun"
#outFilePrefix <- "No.PSMs"


# Input file with the description of the experiment
...
# used to avoid using difficult-to-remember umbers and 
# improve understanding the intent of the work intended

# label position w.r.t. point in graphs
pos.left <- 1
pos.above <- 2
pos.below <- 3
pos.right <- 4
...
# print out some text in a graph, at position x=0 in successive lines
...
# out is the name of the output PDF file
out <- paste(outFilePrefix, 'Analysis', 'pdf', sep='.')
# from now on, save all output on a PDF file in DIN A4 format
pdf(out, paper="a4")
...
#
# 1. get the metadata
phenoData <- read.table(metadata, header=T)
# set column names to use for counts
exprCols <- sub(outFilePrefix, "Line", phenoData$label)
phenoData$label <- sub(outFilePrefix, "Line", exprCols)
rownames(phenoData) <- phenoData$label		#rownames(phenoData) <- rownames(pData(abun))

# 2. get all the experimental data
fnames="Accession"
...
# 1. pre-process all quantitation data
# filter *quantitation* data
qnt <- filterNA(abun)
# processingData() tells us what we have done so far
processingData(qnt)

# 2. pre-process protein-coalesced quantitation data
# get *protein quantitation* data
#    we will group filtered and pre-processed quantitation data by 
#    protein accession code
#	this should have no effect IFF we are alreay working at the
#	protein level
...
# get some partial plots for feedback control
...
# set seed for repeatable science
set.seed(16160423)		# date Shakespeare, Cervantes, Inca Garcilaso died

# choose n random proteins
n.random.prots <- sample( 1:dim(protqnt)[1], n.prots, replace=F )
# get their corresponding expression level
exprs.to.plot <- exprs(protqnt)[n.random.prots, ]
# and their accession names
acc.to.plot <- rownames(protqnt)[n.random.prots]

# choose n colors from a safe color palette (Set1)
...
# 3. normalize the quantitation data
# normalize (starting from qnt, the total quantitation data, 
# not from protqnt, the summarized protein data)
#	try three normalizations
qntS  <- normalise(qnt, "sum")		# sum
qntSV <- normalise(qntS, "vsn")		# sum + vsn
qntV  <- normalise(qnt, "vsn")		# vsn


# 4. do more plots
# plot SD vs means (by row)
vsn::meanSdPlot(qntSV)
vsn::meanSdPlot(qntV)
...
# library(synapter)
# synapterGuide()
#	synapter is used to process Synapt G2 instrument raw data
# to obtain quantitation data
# ...
...
# for peps
#syn_data <- qntV
#syn_data <- topN(syn_data,
#                 groupBy=fData(qntV)$protein.Accession,
#		 n=3)
#nPeps <- nQuants(syn_data, groupBy=fData(qntV)$protein.Accession)
#syn_data <- combineFeatures(syn_data, 
#                            fData(qntV)$protein.Accession,
#			    "sum",
#			    na.rm=TRUE,
#			    verbose=FALSE)
#head(exprs(syn_data)
#head(nPeps)
#exprs(syn_data) <- exprs(syn_data) * (3 / nPeps) 
#head(exprs(syn_data)

# We now want to filter data out based on missing quantitation data,
# retaining proteins that have been quantified in at a least two out of three
# replicates. Filtering based on missing data can be done using the filterNA
# method and a maximum missing data content as defined by pNA. Multiple MSnSet
# instances can be combined with the combine method, which is described in
# details in the MSnbase-demo vignette. The 6 objects have
# appropriate distinct sample names and common feature (protein) names, which
# will be used to properly combine the quantitation data.
#
#comb_data <- combine(exp1, exp2)
#comb_data <- combine(comb_data, exp3)
#dim(comb_data)
#etc...
#comb_data <- filterNA(comb_data, pNA=1/3)
#table(apply(exprs(comb_data), 1,
#            function(.x)sum(!is.na(.x))))
# ... see guide ...
# until we reach a combined MSnSet that contains all the data
# from each of the experiments together (which we already have)

#
# "Trivial" t-test based analysis
#

if (exprColsPrefix == "PSM.") {
    syn_data <- qntSV
} else if (exprColsPrefix == "NormAbun.") {
    syn_data <- qntV		# data is already sum-normalized
}
...
set1.cols <- 1:4
set2.cols <- 5:8
test.data <- exprs(syn_data)[ , c(set1.cols, set2.cols)]
design <- rep(c(1,2), c(length(set1.cols), length(set2.cols)))

nperm <- 1000		# this more or less seems to work (in the literature
			# values of 100.000 or 1.000.000 are suggested, to
			# ensure a stable asymptote is reached, but they do
			# not seem to improve much and can take very, very long.


## Plain p-value calculation
# apply a t-test and extract the p-value
#      Here we compare the four replicas of first sample
#      with the four replicas of the second sample
#      
#      We would need to repeat this analysis for each
#      sample comparison of interest
tt.pv <- apply(exprs(syn_data), 1 ,
            function(x) t.test(x[set1.cols], x[set2.cols])$p.value)

# compute permutation-based identity test for each protein in two groups
#n.permutations <- 1000
#pt.pv <- apply(test.data, 1,
#      p.perm.test
#	  )

# Alternatively, it might be more reasonable to use pairwise.perm.t.test
# or perm.t.test from RVAideMemoire??? 
# (note: perm.t.test is VERY, VERY slow)
#
#n.permutations <- 1000
#ptt.pv <- apply(exprs(syn_data), 1,
#      function(x) perm.t.test(x[set1.cols], 
#                              x[set2.cols], 
#			      nperm=n.permutations, 
#			      progress=F
...
#	    )
#
# What we have got above are the p-values for comparisons, but not the FDR
# estimation or correction. Package 'fdrci' may be useful for that, or
# functions fdr.q.perm(), fdr.od(), fdr.cit() from package 'cit'
#

## get q-values from p-values
##
## using package 'qvalue'
q.values <- qvalue(tt.pv)	# or pt.pv or ptt.pv
#
## using p.adjust
# q.values <- p.adjust(tt.pv, method='BH')	# or pt.pv ot ptt.pv
#
...
### calculate p-value, cutoff and q-values using permutations with permFDP
### (permutation False Discovery Proportion)

# compare both groups using t-test
fpf <- significance.permFDP(test.data, design=design, threshold=0.5)
#fpf <-  significance.permFDP(
#		exprs(syn_data[, set1.cols]), 
#		exprs(syn_data[, set2.cols]))

# repeat FDR estimation with significance.cit
fci <- significance.cit(exprs(syn_data[, set1.cols]), exprs(syn_data[, set2.cols]))
# and with significance.fdrci
ffc <- significance.fdrci(exprs(syn_data[, set1.cols]), exprs(syn_data[, set2.cols]))

# use permFDP
tt.fdp <- fpf

tt.p.sig <- tt.fdp$p.is.significant
		threshold=0.5)
tt.pv <- tt.fdp$p.values
tt.cut <- tt.fdp$p.threshold	# note: use either pvalues + threshold
tt.qv <- tt.fdp$qvalues		# or qvalues, but NOT qvalues + threshold
tt.p.sig <- tt.fdp$p.is.significant
tt.q.sig <- tt.fdp$q.is.significant



pv <- tt.pv
qv <- tt.qv
p.signif.pFDP <- tt.p.sig


## calculate log2 fold-changes
##	See above under t-test
## Note: we have already converted exprs to log2(exprs), hence 
## fold-change (A/B) becomes log2FC = A - B
lfc <- apply(exprs(syn_data), 1 ,
             function(x) mean(x[set1.cols], na.rm=TRUE)-mean(x[set2.cols], na.rm=TRUE))
## create a summary table
syn_res <- data.frame(cbind(exprs(syn_data), 
                            p.value=pv, 
			    q.value=qv, 
			    permFDP=p.signif.pFDP,
			    log2FC=lfc)
		     )
## reorder based on q-values
syn_res <- syn_res[order(syn_res$q.value), ]
# show as a nice table in the screen
#knitr::kable(head(round(syn_res, 3)))

# save
# we may need to save many files, it is a
# good idea to "construct" output file names
# using pre-defined conventions
# here, we use outFilePrefix (which we set to 
# 'NormAbun' at the beginning) to imply that
# this analysis file was generated from Normalized
# Abundance data, and add 'trivial' to indicate the 
# method, and 'all' to indicate we used all the data
out <- paste(outFilePrefix, 'trivial', "all.tab", sep='.')
write.table(syn_res, file=out, row.names=T, col.names=T)

# volcano plot
plot(syn_res$log2FC, -log10(syn_res$q.value),
     col = ifelse(grepl("260UP", colnames(syn_res)),
       "#4582B3AA",
       "#A1A1A180"),
     pch = 19,
...
legend("topright", c("260UP", "others"),
       col = c("#4582B3AA", "#A1A1A1AA"),
       pch = 19, bty = "n")
       
# heatmap
msg <- paste("t-test based trivial analysis of", exprColsPrefix)
...
# We can use any of the protqnt/qntS/qntSV/qntV datasets 
...
if (from.abundances == T) {
    ...
    #data_se <- as(qntSV, "SummarizedExperiment")
    #exprs(data_se) <- log2(exprs(data_se))
    data <- qntSV
...
data_diff <- test_diff(data_imp, type = "control", control = "L260UP")
...
data_diff_manual <- test_diff(data_imp, type = "manual", 
                              test = c("L260UP_vs_L260P", "L260P_vs_L260UP"))
...
DEP::plot_volcano(dep, contrast = "L260P_vs_L260UP", label_size = 4, 
	add_names = TRUE)
...
plot_cond(dep)
# this is of little use if we only have one condition
...
# Save analyzed data
out <- paste(outFilePrefix, "DEP", "RData", sep='.')
save(data_se, data_norm, data_imp, data_diff, dep, file=out)
# These data can be loaded in future R sessions using this command
load(out)

# save table
out <- paste(outFilePrefix, "DEP", "diff.tab", sep='.')
write.table( df_wide[df_wide$significant==T, ], file=out, row.names=T, col.names=T)

# simplify
out <- paste(outFilePrefix, "DEP", "diff.simple.tab", sep='.')
write.table( df_wide[df_wide$significant==T, c("name", "Description", "L260P_vs_L260UP_diff", "L260P_vs_L260UP_p.adj")], 
...
out <- paste(outFilePrefix, "eR.signif", "p", p.cutoff, "tab", sep='.')
...
out <- paste(outFilePrefix, "eR.signif", "padj=0.01.tab", sep='.')
write.table( sigpa, file=out, row.names=F, col.names=T)

# simplify
out <- paste(outFilePrefix, "eR.signif", "p", p.cutoff, "simple.tab", sep='.')
write.table(sigp[ , c("accession", "Description", "p.value", "p.adjust", "LogFC")],
	   file=out, row.names=F, col.names=T)

out <- paste(outFilePrefix, "eR.signif", "padj=0.01.simple.tab", sep='.')
...
out <- paste(outFilePrefix, "eR.mark", "p", alpha.cut, "tab", sep='.')
write.table(eR.wide, file=out, row.names=F, col.names=T)

out <- paste(outFilePrefix, "eR.mark", "p", alpha.cut, "simple.tab", sep='.')
