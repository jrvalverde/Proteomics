...
cran_packages <- c("RColorBrewer", "ggplot2", "reshape2", "vsn", 
		"dplyr", "remotes", 'knitr', 'RVAideMemoire', 'fdrci')
...
#	this should have no effect as we are alreay working at the
#	protein level
...
lfc <- apply(exprs(syn_data), 1 ,
             function(x) mean(x[1:4], na.rm=TRUE)-mean(x[5:8], na.rm=TRUE))
## create a summary table
syn_res <- data.frame(cbind(exprs(syn_data), p.value=pv, q.value=qv, log2FC=lfc))
